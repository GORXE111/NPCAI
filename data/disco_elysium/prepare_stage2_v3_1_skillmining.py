...
OUTPUT_DIR = "D:/AIproject/NPCAI/data/disco_elysium/training_stage2_v3_1"
...
# Balance: 50/50 positive/negative to prevent over-calling reflex
positives = [s for s in samples if s["type"] != "empty"]
negatives = [s for s in samples if s["type"] == "empty"]
# Target: same number of negatives as positives → 50% empty (prevent over-calling)
...
print(f"  Negatives sampled: {len(sampled_negs)} (target 50/50)")
