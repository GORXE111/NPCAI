...
OUTPUT_DIR = "D:/AIproject/NPCAI/data/disco_elysium/training_stage2_v5_0"
...
# v5.0 balance: SKEW toward empty (1.3x positives) to hold suppression.
# Rationale: v3.1 @ 50/50 -> Suppress 0.30; v4.0 @ 50/50 (more positive diversity)
# -> Suppress collapsed to 0.13 (over-calling). More positive coverage makes the
# model call more eagerly, so we counterbalance with proportionally more empties.
...
target_neg = int(len(positives) * 1.3)  # v5.0: skew empty to hold suppression
sampled_negs = random.sample(negatives, min(target_neg, len(negatives)))
balanced = positives + sampled_negs
random.shuffle(balanced)
print(f"After balance (pos:neg ~ 1:1.3): {len(balanced)} (pos {len(positives)} + neg {len(sampled_negs)})")
