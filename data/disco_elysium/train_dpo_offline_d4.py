...
# D3: policy-init AND reference are the WARM-START LoRA (seeds the new tools).
# DPO anchors to warm-start (protects new-tool capability) while preservation
# pairs repair the SFT-induced drift on old categories.
STAGE2_LORA = os.path.expanduser("~/npcllm/checkpoints/kim_q35_2b_stage3_warmstart_d3/lora")
DATA_DIR = os.path.expanduser("~/npcllm/data_kim_dpo_d4")
REF_LP_FILE = os.path.join(DATA_DIR, "ref_logprobs_d4.json")
OUT = os.path.expanduser("~/npcllm/checkpoints/kim_q35_2b_stage3_dpo_d4")

LR = 5e-7
BETA = 0.07   # D4: looser than D3's 0.1 to let the heavy suppress signal pull back over-calling
EPOCHS = 1
GRAD_ACCUM = 8
