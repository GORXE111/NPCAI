...
OUTPUT_DIR = "D:/AIproject/NPCAI/data/disco_elysium/training_dpo_v3_1_D2"
os.makedirs(OUTPUT_DIR, exist_ok=True)
random.seed(44)  # D2: different seed
...
anchor_pairs = []
for s in v31_by_type["skill_check"][:40]:  # D2: 40 skill anchor (was 30)
    msgs = s["messages"]
...
for s in v31_by_type["show_character"][:25]:  # D2: 25 (was 20)
    msgs = s["messages"]
    try:
...
for s in v31_by_type["present_choices"][:15]:  # D2: 15 (was 10)
    msgs = s["messages"]
    try:
        cd = json.loads(msgs[2]["content"])
        rejected = json.dumps({"dialogue": cd.get("dialogue", "..."), "tool_calls": []}, ensure_ascii=False)
...
suppress_pairs = []
for s in empty_samples[:60]:  # D2: 60 (was 50)
    msgs = s["messages"]
    try:
        cd = json.loads(msgs[2]["content"])
