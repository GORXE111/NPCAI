"""
Stage 3a (D3 warm-start SFT): lift set_expression / present_choices probability
from ~0 to non-zero on top of v3.1, so the subsequent DPO (ref=warm-start) can
reweight them. ONLY positive new-tool targets + anchors, NO suppress negatives
(that was the v3.1.1 collapse cause). 1 epoch — just enough to seed the tools.
"""
...
DATA_DIR = os.path.expanduser("~/npcllm/data_kim_warmstart_d3")
OUT = os.path.expanduser("~/npcllm/checkpoints/kim_q35_2b_stage3_warmstart_d3")

LR = 1e-5
DROPOUT = 0.15
EPOCHS = 1          # D3 warm-start: just seed the new tools, don't overfit
PATIENCE = 1
WEIGHT_DECAY = 0.05
GRAD_ACCUM = 8
