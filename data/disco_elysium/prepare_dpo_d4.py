"""
Stage 3 v3.1.D2 — STRONGER branch/emotion signal on top of the v3.1.D recipe.

v3.1.D result: F1 0.639→0.660, all old categories preserved, BUT branch/emotion
still 0 — the β=0.1 KL anchor that protects old categories also damps the new
signal below threshold.

v3.1.D2 fix (two levers, same goal):
- β 0.1 → 0.05 (looser KL, lets new signal cross threshold)   [in train script]
- More branch/emotion pairs (50/40 → 70/60) — stronger signal
- More anchor/suppress preservation (60/50 → 80/60) — compensate the looser KL
- reference + policy-init still = v3.1 (directly comparable to D)

Pair composition (270 target):
   70 branch    — chosen=present_choices, rejected=show_character  (v3.1's mistake)
   60 emotion   — chosen=set_expression,  rejected=skill_check/show (v3.1's mistake)
   80 anchor    — chosen=v3.1's correct call, rejected=empty       (don't drift to silent)
   60 suppress  — chosen=empty, rejected=random wrong tool          (don't over-call)

Crucial: bidirectional balance maintained
...
OUTPUT_DIR = "D:/AIproject/NPCAI/data/disco_elysium/training_dpo_d4"
os.makedirs(OUTPUT_DIR, exist_ok=True)
random.seed(46)  # D4: different seed
...
mined_branch_selected = dedup_mined[:55]  # D2: was 35
...
EMOTION_SEEDS = [
    ...
    # D2: 20 additional emotion scenarios (DEBench-disjoint, real DE elements)
    (["Measurehead: \"Your race is written in the angle of your skull.\""],
     "I've heard enough of this nonsense.", "cold"),
    (["Garte: \"You owe me for the window, the room, and the noise.\""],
     "We'll settle the bill. Itemize it.", "businesslike"),
    (["[Gunfire erupts in the courtyard. Then silence.]",
      "Detective: \"Is it over? Are they down?\""],
     "Check your weapon. Don't move yet.", "shaken"),
    (["Cindy: \"That's my tag on the wall. SKULL. Got a problem?\""],
     "It's vandalism. But that's not why we're here.", "unimpressed"),
    (["Iosef: \"I shot him. From the island. Fifty years too late.\""],
     "So that's how it ends. Tell me everything.", "grave"),
    (["Soona: \"The Pale is eating the world from the edges inward.\""],
     "And nobody can stop it. Go on.", "curious"),
    (["Detective: \"My mind keeps spiraling. I can't make it stop.\""],
     "Then lean on me until it does. I'll hold the line.", "worried"),
    (["Detective: \"Remind me again — am I actually a cop?\""],
     "Yes. Lieutenant-double-yefreitor. We'll go slowly.", "patient"),
    (["Detective: \"Do you believe in innocence, Kim? Real innocence?\""],
     "I believe in evidence. Innocence is what we protect.", "reflective"),
    (["Detective: \"Klaasje's gone. Out the window, into the bay.\""],
     "Of course she is. We were always one step behind.", "resigned"),
    (["[The body has been in the heat for a week.]",
      "Detective: \"God, the smell. How do you stand it?\""],
     "You breathe through your mouth and you work the case.", "clinical"),
    (["Detective: \"Ruby jumped. Right off the boardwalk. She's gone.\""],
     "We pushed too hard. I'll carry that.", "regretful"),
    (["Cuno: \"Cuno does what Cuno wants! Cuno's on speed!\""],
     "He's a child poisoning himself. We should intervene.", "concerned"),
    (["Evrart: \"Help an old man out of his chair, would you, officer?\""],
     "You can manage. We both know that.", "wary"),
    (["[Music drifts from the abandoned church.]",
      "Detective: \"Dance with the kids, Kim? Just once?\""],
     "I... will observe. You enjoy yourself.", "hesitant"),
    (["Detective: \"I found my gun, Kim. It was in the locker all along.\""],
     "Thank God. Holster it and let's not lose it again.", "relieved"),
    (["[They cut the hanged man down from the tree.]",
      "Detective: \"Easy. Lay him down gently.\""],
     "A week he hung there. He deserves some dignity now.", "somber"),
    (["Annette: \"Mum left me to mind the books. I'm not scared.\""],
     "You're very brave. Is there an adult nearby?", "gentle"),
    (["Detective: \"That's it. Every thread ties together. We solved it.\""],
     "We did. Good work, detective. Now the paperwork.", "satisfied"),
    (["Detective: \"The tribunal's coming for us. Mercenaries, armed.\""],
     "Then we hold this position and we stay calm.", "steeled"),
]

emotion_pairs = []
for i, (ctx_lines, kim_d, emo) in enumerate(EMOTION_SEEDS[:60]):  # D2: was 40
...
anchor_pairs = []
for s in v31_by_type["skill_check"][:25]:  # D4: 25 (cut from 40 — warm-start over-calls)
    msgs = s["messages"]
...
for s in v31_by_type["show_character"][:15]:  # D4: 15 (cut from 25)
    msgs = s["messages"]
    try:
...
for s in v31_by_type["present_choices"][:0]:  # D4: 0 (present_choices is a TARGET tool, not anchor)
    msgs = s["messages"]
    try:
        cd = json.loads(msgs[2]["content"])
        rejected = json.dumps({"dialogue": cd.get("dialogue", "..."), "tool_calls": []}, ensure_ascii=False)
...
suppress_pairs = []
for s in empty_samples[:120]:  # D4: 120 (was 60) — main lever to pull back over-calling
    msgs = s["messages"]
    try:
        cd = json.loads(msgs[2]["content"])
