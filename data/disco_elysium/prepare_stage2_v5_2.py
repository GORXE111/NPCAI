...
OUTPUT_DIR = "D:/AIproject/NPCAI/data/disco_elysium/training_stage2_v5_2"
...
# v5.2 balance: MINIMAL nudge from v5.0 (F1 0.641, the known-good point).
# v5.0 @ 44% empty over-called (Suppress 0.10); v5.1 @ 54% empty + skill halved
# over-corrected to under-calling (Recall 0.06). v5.2 moves ONE small step from
# v5.0 toward suppression: skill_check 453->400 (tiny trim), empty 44%->48%.
CAPS = {"skill_check": 400, "show_character": 9999, "present_choices": 100, "set_expression": 55}
by_type = {}
for s in samples:
...
# empty ~48% of total: target_neg = positives * (0.48/0.52) ~= 0.92
target_neg = int(len(positives) * 0.92)
