...
We evaluate **twelve ablation configurations** spanning two model scales (0.8B and 2B) on DEBench v1 (130 scenarios across persona, tool-selection, and tool-suppression). Two headline findings: (1) **model scale is the decisive factor for proactive tool selection at this regime — Qwen3.5-2B achieves Tool F1 = 0.639 versus 0.8B's best of 0.105, a 6× improvement** while the gap from in-domain SFT data engineering at 0.8B is comparatively small; and (2) **for refining a near-saturated tool-calling policy, KL-anchored DPO strictly dominates continued SFT — DPO lifts our 2B system to Tool F1 = 0.660 (our best) while preserving every previously-learned category, whereas every continued-SFT attempt catastrophically forgot them** (§7.6, §7.7).

**Table 2.** DEBench v1 results across 12 configurations. Bold = best in column. The final reported system is **Stage 3 DPO v3.1.D (2B)**.
...
| Stage 2 v4.0 (2× data) | 2B | 0.653 | 0.688 | 0.623 | 0.133 | 1.000 | 0.980 |
| Stage 3 SFT-cont. v3.1.1 | 2B | 0.073 | 1.000 | 0.038 | 0.967 | 0.980 | 0.980 |
| Stage 3 SFT-cont. v3.1.2 | 2B | 0.107 | 0.182 | 0.075 | 0.167 | 0.880 | 0.980 |
| **Stage 3 DPO v3.1.D** | **2B** | **0.660** | **0.727** | **0.604** | 0.300 | 1.000 | 0.980 |

**Per-category Tool F1 (final system, Stage 3 DPO v3.1.D, 2B)**: evidence P=0.67/R=0.40, social P=1.00/R=0.40, scene P=0.60/R=0.60, combined P=1.00/R=0.50, filler P=1.00/R=1.00. The branch (present_choices) and emotion (set_expression) categories remain at P=0/R=0 — directly attributable to their near-absence in the v3.1 training set (20/0 samples respectively out of 677); the DPO preference signal we added for them was too weak to cross the learning threshold under the KL constraint that protects the other categories (§7.7). Relative to the Stage 2 v3.1 base it refines, **DPO improved overall F1 (0.639→0.660), precision (0.705→0.727), recall (0.585→0.604), and the scene category (P=0.50/R=0.40→P=0.60/R=0.60), while holding Suppression, JSON validity, and no-break exactly constant** — the signature of a refinement that adds skill without distributional drift. Reproducibility was verified: a second DEBench run on the v3.1 checkpoint produces bit-identical results under greedy decoding.
...
The result is our final system. DPO lifted v3.1 to **Tool F1 = 0.660** (+0.021) and, decisively, **held every previously-learned category constant or improved** — filler P/R = 1.00/1.00, combined 1.00/0.50, Suppression exactly 0.300, JSON 1.00, no-break 0.98 — where both SFT variants had destroyed them. Validation preference accuracy moved gently from 0.15 to 0.65 over one epoch (no sign of the over-fitting that preceded our 0.8B collapses). Branch and emotion stayed at 0: under β=0.1 the KL anchor that protects the old categories also damps the new signal below threshold. This is a genuine limitation, but it isolates the remaining problem to *signal strength under a fixed KL budget* rather than the catastrophic-interference problem that defeats SFT.
...
**Methodological takeaways.** (1) For *refining* a near-saturated tool policy, prefer KL-anchored DPO over continued SFT: the reference-model KL term is precisely the mechanism that prevents catastrophic forgetting, which unconstrained SFT cannot avoid at small trainable-parameter scale. (2) DPO preference data must be **directionally balanced** — inventory whether each perturbation teaches "add tool" or "remove tool" and keep them near-parity, on *both* the chosen and rejected sides. (3) Preference-pair Val Acc is an unreliable proxy for generation quality (0.8B reached 98.57% then collapsed); always confirm on a behavioral benchmark. (4) Offline (precomputed-reference) DPO makes 2B KL-anchored preference training feasible in 16 GB. We release all checkpoints — the collapsed 0.8B v1, the recovered 0.8B v2, both failed 2B SFT-continuation variants, and the final 2B DPO v3.1.D — as a documented trajectory.
...
**Concrete contributions delivered.** All training runs on a single Apple M4 Mac (16GB): persona-SFT achieves Val Loss 0.89 (2B) on 1,127 cleaned Kim utterances; tool-augmented SFT reaches 100% JSON schema validity and Tool F1 = 0.639 at 2B; and a final KL-anchored DPO stage lifts the system to **Tool F1 = 0.660** — our best — without disturbing any previously-learned category. Two scale findings frame the recipe: (1) there is a sharp 0.8B→2B capacity threshold (best 0.8B F1 = 0.105 vs 2B = 0.639, 6.1×) below which no SFT/DPO recipe we tried crossed F1 = 0.11; and (2) for *refining* a near-saturated policy, the post-training objective is decisive — continued SFT catastrophically forgets (F1 0.07–0.11), DPO preserves and improves. Per-turn inference latency is ~1.5s, well within interactive-game requirements and 4.7× faster than serving Qwen3.5-9B on the same hardware.

**Methodological contributions.** Beyond the system, we document (1) a target-content cleaning step for DE training data that improved Val Loss 13% while reducing dataset size 29% — quality over quantity; (2) the **SFT-continuation vs KL-anchored-DPO contrast** for in-place policy refinement, with the directional-balance and offline-reference recipes that make 2B DPO succeed in 16 GB; (3) a documented family of negative results — 0.8B DPO directional collapse, and two 2B SFT-continuation forgetting modes (over-suppression and over-`present_choices`); (4) DEBench, a 130-scenario *Disco Elysium*-grounded benchmark with automated scoring of persona, tool-selection, and tool-suppression dimensions.

**Open release.** We release: (i) all trained LoRA checkpoints — 0.8B Stage 1/2/3-v1/3-v2 and 2B Stage 1/2-v3.1/3-SFT-v3.1.1/3-SFT-v3.1.2/3-DPO-v3.1.D — each ~4–6 MB; (ii) the Unity visual-novel environment with 18 tool handlers; (iii) DEBench v1 with scoring scripts; (iv) the MPS-compatibility patch `qwen35_mps_fix.py` enabling Qwen3.5 training/inference on Apple Silicon; (v) full data-preparation and training pipelines including the offline-DPO precompute step; (vi) a project knowledge base documenting the v0→v3 direction evolution. Everything is at https://github.com/GORXE111/NPCAI.

**Limitations.** branch (`present_choices`) and emotion (`set_expression`) remain at F1 = 0 in the final system: under the β=0.1 KL budget that protects the learned categories, the preference signal for these two under-represented tools is too weak to cross the learning threshold — isolating the remaining gap to signal strength rather than catastrophic interference. CPDC 2025 official scoring and BFCL V4 numbers are planned for §7.3 and §7.4 once the evaluation pipeline completes. A live human-subject study comparing the final DPO system to a cloud baseline on Prolific is future work.
