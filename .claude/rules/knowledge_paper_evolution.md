...
## 2026-07-07 / Stage 3 完整 refinement 研究收官：一条 Pareto 前沿 + SFT 混沌

### 背景
D2 (DPO, F1 0.680) 是最佳但 branch/emotion=0。用户持续追问能否突破，尤其 emotion。做了一整轮 D3-D7 + v5.0-v5.2（~10 个新系统），把 2B+LoRA 框架彻底走完。

### 完整实验矩阵（全 2B）
| 系统 | 方法 | F1 | Suppress | emotion | 关键 |
|------|------|:--:|:--------:|:-------:|------|
| v3.1 | Stage2 SFT | 0.639 | 0.30 | 0 | 基线 |
| v4.0 | Stage2 2×数据 | 0.653 | 0.13 | 0 | 过调 |
| **D2** | DPO(ref=v3.1) | **0.680** | 0.27 | 0 | **max-F1** |
| D5 | DPO 召回导向 | 0.673 | 0.27 | 0 | 召回推不动(0.62→0.62) |
| D3 | warmSFT→DPO | 0.122 | 0.0 | **0.80** | emotion 突破但全崩 |
| D4 | warmSFT→DPO重抑制 | 0.094 | 0.0 | 0.80 | DPO修不了坏ref |
| **v5.0** | 全工具SFT+合成 | 0.641 | 0.10 | **0.20** | **max-coverage, evidence 1.0** |
| D7 | DPO(ref=v5.0) | 0.621 | 0.10 | 0.20 | DPO修不了v5.0过调 |
| v5.1 | 重平衡empty多数 | 0.09 | 0.40 | 0.25 | 过度沉默 |
| v5.2 | 重平衡中点47% | 0.00 | 1.0 | 0 | **完全塌陷(违反直觉)** |

### 四个核心发现（都写进论文 §7.7）

**1. DPO 无法引入 reference 零概率工具**（D2/D5 + β=0.05 变体都试过）
- set_expression 在 v3.1 数据 0 样本 → π_ref≈0 → log(π/π_ref) 梯度退化 → 拉不起来
- 这是 DPO 数学本质，不是超参问题。present_choices(16样本)也几乎拉不动

**2. DPO 无法修复自己 reference 的缺陷**（D4 + D7 两次确认）
- D4 ref=warm-start(过调)、D7 ref=v5.0(过调) → KL 把 policy 钉在坏 prior 上 → 再多 suppress pairs 也拉不回
- warm-start SFT 注入 emotion(0.80!) 但同时把 base 推成"什么都调"，DPO 保护了这个 corruption

**3. 合成数据做对了确实有正反馈**（v5.0，纠正 v3.3 的失败）
- v3.3 失败=6.5词(短)+主导小数据+扰动冻结最优
- v5.0 成功=14词(长度匹配)+占1510的3.8%(少数)+从头训练 → emotion 0→0.20, evidence→1.0
- 代价：换了一组弱项(scene→0, Suppress→0.10)。是 Pareto 移动不是严格更好

**4. SFT 工具调用对数据配比是"混沌"**（v5.0/v5.1/v5.2）
- empty 44%→过调, 54%→过静, **47%(中点)→完全塌陷**(比两头都差, 非单调!)
- data-balance → generation-behavior 的映射不平滑不单调 → **没有可搜的甜点**
- v3.1/v5.0 的好分是"幸运着陆"，不是可调优化到的最优

### 最终定论：Pareto 前沿，无单一赢家
- **D2 (F1 0.680)**: max-F1, 保所有旧类别, 但 emotion/branch=0
- **v5.0 (F1 0.641, emotion 0.20)**: max-coverage, evidence 满分, 但 scene=0 + 过调
- 两者互不支配。所有融合尝试(D3/D4/D7/v5.1/v5.2)都退化不是合并
- 结论：2B+LoRA+~1.5K数据下，单个工具类别可达，但**全类别兼得超出该容量/数据规模**

### 假设 vs 现实
- **假设**: 能找到一个系统同时 F1 高 + emotion 非零
- **现实**: 是一条前沿，不是一个点。每个杠杆只在前沿上滑动或掉出前沿
- **打脸**: 我一度以为 v5.2 中点能插值出甜点 → 反而完全塌陷，证明地形混沌

### 关键技术产物
- **offline DPO**: 双 2B 在 16GB OOM(step 143/180) → Phase1 只载 ref 预计算 logprobs 存盘释放, Phase2 只训 policy, 峰值 10GB。2B KL-anchored DPO 在消费硬件可行的配方
- **SCP 竞态坑**: 后台 SCP 没传完就启动训练 → JSON 截断崩溃。必须校验行数+JSON valid 后才启动(v5.1/v5.2 栽过)

### 下次注意（actionable）
1. **要引入全新工具(零概率) → 只能 SFT，不能 DPO**。且要从头训练+长度匹配+占小比例
2. **DPO 只精修 reference 的优点，修不了缺点** → ref 必须是个好起点
3. **SFT 工具配比是混沌的 → 不要把 balance-tuning 当优化循环**，跑出好点就复现验证锁定，别再拧
4. **Val Acc/Val Loss 全程是陷阱**：v5.x 到 0.89-0.95、0.8B DPO 98%，generation 全崩。只信 DEBench
5. **Mac 跨天训练靠 caffeinate 防休眠**，否则一个 SFT 能拖好几天
6. **每次 SCP 后校验完整性**(wc -l + json.loads)再启动，避免竞态崩溃
